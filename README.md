...
>
> **Case study:** ASML ADR (NYSE) vs ASML Amsterdam shares with EUR/USD.
...
Bloomberg delivers FX implied vols as ATM straddle + 25&Delta; butterfly spread + 25&Delta; risk reversal. The individual wing vols are:
...
│       ├── cleaned_dataset.csv            — Aligned, business-day index, 2491 × 22, zero NaNs
...
- Forward-filled across market holidays (≤ 3 consecutive days)
- Zero NaNs after processing
...
| `delta_var_1m` | Raw numerator $\sigma^2_\text{adr} - \sigma^2_\text{loc} - \sigma^2_\text{fx}$ at 1M |
...
The raw CSVs and processed datasets are already committed, so you can inspect
outputs immediately. To regenerate everything from scratch:
...
sources → forward-fill holiday gaps (≤ 3 days) → drop any remaining NaN rows.

### `src/data_ingestion/ingest_csv_pipeline.py`
**Step 1 script.** Calls `loader.py` and `cleaner.py` for the three ASML/EURUSD
CSVs, prints a structured status report (row counts, column names, dtypes, first
5 rows), and saves `cleaned_dataset.csv`.
...
Reconstructs 25Δ call and put vols from ATM + butterfly + risk reversal using the
standard Garman-Kohlhagen conventions. Auto-detects pair prefixes from column names.
